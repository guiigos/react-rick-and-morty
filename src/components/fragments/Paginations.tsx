...
const Button: React.FC<React.HTMLProps<HTMLInputElement> & ButtonProps> = ({ 
  first = false, 
  last = false, 
  current = false, 
  ...rest
}): React.ReactElement => {
  function createStitches(): React.ReactElement {
  ...
  return (
    <>
      ...
      <input 
        type="button" 
        className={`p-1 m-1 w-8 h-8 cursor-pointer font-bold bg-yellow-600 rounded-lg ${current && "text-red-700"}`} 
        {...rest} />
      {first && createStitches()}
    </>
  );
}
    ...
    <div className="text-center">
      ...
      {numbers.map((value: number) => (
        <Button 
          ...
          onClick={() => onPage(value)} value={value} />
      ))}
